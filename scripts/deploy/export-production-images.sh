...
echo ""
echo "▶ Copy archive từ sandbox về local:"
echo "   sudo scp user@<sandbox-node>:${OUTPUT_FILE} ."
echo "▶ Copy local về production:"
echo "   tsh scp $(basename "${OUTPUT_FILE}") user@server:/home/user/"
echo "▶ Trên VM production:"
echo "   ARCHIVE=/home/thuyldx/$(basename "${OUTPUT_FILE}")"
echo "   sha256sum \"\${ARCHIVE}\""
echo "   gzip -dc \"\${ARCHIVE}\" | docker load"
echo "   docker image ls"
echo "   docker compose config --images | sort -u"
echo "   docker image inspect apache/apisix:3.17.0-debian --format '{{.Id}}  {{.Os}}/{{.Architecture}}'"
echo "   docker run --rm apache/apisix:3.17.0-debian apisix version"
